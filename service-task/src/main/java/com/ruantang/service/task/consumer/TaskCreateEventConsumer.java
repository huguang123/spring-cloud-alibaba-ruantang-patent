package com.ruantang.service.task.consumer;
...
import com.ruantang.service.task.event.TaskCreateEvent;
...
public class TaskCreateEventConsumer {
...
//    private final TaskService taskService;
     ...
     */
    @RabbitListener(
            queues = RabbitConstants.TASK_CREATE_QUEUE,
            ackMode = "MANUAL"
    )
    public void consumeTaskCreateEvent(
            Message message,
            Channel channel,
        ...
        try {
            ...
            // 2. 反序列化消息体
            TaskCreateEvent event = deserializeMessage(message);
    ...
    private void validateMessageProtocol(Message message) {
        ...
        if (!MessageType.TASK_CREATED.equals(actualType)) {
            throw new UnexpectedMessageTypeException(
                    "Expected TASK_CREATED but received: " + actualType
            );
        }
    }
     ...
     * 反序列化消息体
     */
    private TaskCreateEvent deserializeMessage(Message message) {
        try {
            return objectMapper.readValue(message.getBody(), TaskCreateEvent.class);
        } catch (IOException e) {
     ...
     * 处理业务逻辑
     */
    private void processBusinessLogic(TaskCreateEvent event) {
        log.info("创建新任务 - 案件ID: {}, 类型: {}, 负责人: {}",
                event.getCaseId(), event.getTaskType(), event.getAssigneeId());
